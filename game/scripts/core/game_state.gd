...
## M2-7(F5-2 게시판 일일 의뢰) 신설. "게임 내 날짜" 정수 카운터 — 실제 낮/밤·달력
## 시스템은 아직 없어(월드는 프로토타입 청크뿐) 지금은 QuestSystem이 일일 의뢰
## 재추첨 시드로만 쓰는 순수 카운터다. 0에서 시작하며, 증가 트리거(수면/자정 등)는
## 아직 어떤 시스템도 호출하지 않는다 — 실제 하루 경과 판정(day/night 사이클, 수면
## 상호작용 등)이 확정되면 그 지점에서 이 값을 올리기만 하면 된다(game-designer/
## godot-engineer TODO, 완료 보고 질문 목록 참고). 세이브에 포함된다.
var day_index: int = 0
