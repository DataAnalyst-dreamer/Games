...
| 2026-09-10 | 주인공 "핀" 32×48 원작 스프라이트 시안(idle/walk/attack/hurt, 4방향) | 프로젝트 자체(art-director 서브에이전트) | **저작권 프로젝트 소유** — `tools/art/build_fin_sprite.py`(자체 작성 파이썬 절차적 도트 생성 스크립트)가 팔레트 인덱스 기반 좌표 상수로부터 픽셀 단위로 새로 그린 오리지널 그래픽. 기존 Ninja Adventure Knight 등 외부 팩을 확대·트레이싱·파생시키지 않았으며, 색상만 하틀랜드 32색 팔레트(`docs/art/art-bible.md` §3)의 부분집합을 재사용(하틀랜드 지역 룩 통일 목적) | 프로젝트 내부 제작(`tools/art/build_fin_sprite.py` 재실행 명령, 상세는 `docs/art/character-fin-spec.md`) | `game/assets/sprites/characters/fin/` (`fin_idle.png`, `fin_walk.png`, `fin_attack.png`, `fin_hurt.png`) | 크레딧 불필요(자체 저작) |
