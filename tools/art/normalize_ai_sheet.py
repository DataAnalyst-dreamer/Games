#!/usr/bin/env python3
"""
normalize_ai_sheet.py — AI 도트 생성 툴(PixelLab.ai / Ludo.ai 등) 산출물을
《이슬란드 연대기》 캐릭터 스프라이트 규격(기본: 32×48 셀, 발밑 피벗 (16,43),
행=방향[down/left/up/right] · 열=프레임, 투명 배경)으로 정규화한다.

배경 문서: docs/art/ai-sprite-pipeline.md, docs/art/art-bible.md(§1·§3),
docs/brd/04-decisions.md D-130~D-135.
...
2) frames : 폴더 하나. 그 아래 down/ left/ up/ right/ 4개 하위 폴더에
            ...
            (PixelLab/Ludo가 방향별 개별 PNG로 내보내는 경우 이 모드를 쓴다.)
...
- 프레임 간 바운딩박스(발밑 앵커) 지터 리포트(--out에 <name>_report.json/.txt)
- 발밑 피벗 정렬: 각 프레임의 불투명 픽셀 바운딩박스 하단-중앙을 목표 셀 안의
  --pivot-x/--pivot-y 좌표로 옮겨 붙인다(비율 유지 축소/확대, NEAREST).
- 4배 확대 미리보기 PNG(그리드선 포함) + 방향별 애니메이션 미리보기 GIF 생성.

예시
----
# PixelLab에서 뽑은 walk 8프레임×4방향 단일 시트(소스 셀 24×24, 배경 흰색)를 정규화
python3 tools/art/normalize_ai_sheet.py \\
    --mode sheet --input inbox/fin_walk_raw.png \\
    --src-cell-w 24 --src-cell-h 24 --cols 8 --rows 4 \\
    --name chr_fin_walk --transparent-bg --bg-color 255,255,255 \\
    --quantize --out game/assets/sprites/characters/fin/_normalized

# Ludo.ai에서 방향별 폴더로 뽑은 attack 6프레임을 정규화(배경이 이미 투명한 PNG)
python3 tools/art/normalize_ai_sheet.py \\
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--src-cell-w", type=int, default=32, help="sheet 모드: 소스 셀 너비(px)")
    p.add_argument("--src-cell-h", type=int, default=48, help="sheet 모드: 소스 셀 높이(px)")
    ...
    p.add_argument("--cell-w", type=int, default=DEFAULT_CELL_W, help="출력 셀 너비(기본 32, art-bible.md §1)")
    p.add_argument("--cell-h", type=int, default=DEFAULT_CELL_H, help="출력 셀 높이(기본 48, art-bible.md §1)")
    p.add_argument("--pivot-x", type=int, default=DEFAULT_PIVOT_X, help="출력 셀 안에서 발밑 피벗 X (기본 16)")
    p.add_argument("--pivot-y", type=int, default=DEFAULT_PIVOT_Y, help="출력 셀 안에서 발밑 피벗 Y (기본 43)")
