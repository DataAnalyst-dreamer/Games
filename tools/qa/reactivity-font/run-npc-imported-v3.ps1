...
Get-FileHash -LiteralPath $translation,(Join-Path $repo 'game/scripts/ui/hud.gd'),(Join-Path $repo 'game/localization/ui_ko.csv'),(Join-Path $repo 'tools/qa/reactivity-font/npc-runtime-imported-v3.gd'),(Join-Path $repo 'tools/qa/reactivity-font/npc-probe-v2.gd'),(Join-Path $base 'probe/project.godot'),(Join-Path $base 'csv-import/project.godot'),(Join-Path $base 'csv-import/ui_ko.csv'),$engine,$PSCommandPath | Select-Object Path,Hash | ConvertTo-Json | Set-Content (Join-Path $session 'inputs.json') -Encoding utf8
$runtimeFailure = $null
try {
    Run-NpcChild 'runtime-v3' @('--headless','--audio-driver','Dummy','--path',(Join-Path $base 'game'),'--script',(Join-Path $repo 'tools/qa/reactivity-font/npc-runtime-imported-v3.gd')) 'NPC_RUNTIME_IMPORTED_RESULT PASS=286 FAIL=0'
} catch { $runtimeFailure = $_ }
finally {
