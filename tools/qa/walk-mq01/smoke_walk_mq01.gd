...
func _ready() -> void:
	...
	var expected := "C:/Users/freer/ClaudeProject/make-passive-income/reviews/games-2026-09-12/runtime/autoload-exit-diag-monsters-v1/appdata/AutoloadExitDiagMonsters"
	...
	print("WALK_START position=",player.global_position," monsters=",get_tree().get_nodes_in_group("monsters").size())
	...
	if not await _walk(Vector2(-399,190)): return
	await _press(KEY_E)
