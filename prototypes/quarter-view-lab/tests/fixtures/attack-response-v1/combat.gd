...
func _update_player(delta: float) -> void:
	...
	player.tick_clock(delta)
	match player.phase:
		"idle":
			if input != Vector2.ZERO:
				player.facing = input
			player.move_body(delta, input * float(config.move_speed))
		"startup":
			player.move_body(delta, Vector2.ZERO)
			if player.phase_elapsed >= player.phase_duration:
		...
		"active":
			...
			_check_player_sweep(start, end)
			# Do not replace the contact pose in the same tick as a successful hit.
			if freeze_remaining <= 0.0 and player.phase_elapsed >= player.phase_duration:
...
func _check_player_sweep(start: float, end: float) -> void:
	for slime in slimes:
		if slime.is_dead or player.hit_targets.has(slime.get_instance_id()):
			continue
		var point := swept_contact(player, slime, start, end)
...
func swept_contact(attacker: QuarterActor, target: QuarterActor, start: float, end: float) -> Vector2:
	...
	var steps := maxi(1, int(ceil(absf(end - start) * rad_to_deg(attacker.sweep_radians) / 3.0)))
	for index in range(steps + 1):
		var direction := Vector2.from_angle(attacker.blade_angle(lerpf(start, end, float(index) / steps)))
